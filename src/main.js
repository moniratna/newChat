function Chat() {
    ...
    this.sendFriend =''; // The user socketId of the current chat friend
    this.sendChatGroup =''; // The roomId of the group chat of the current chat
...
Chat.prototype = {
    ...
    initSocket(userName, userImg) {
        ...
        window.socket.on('receiveMsgGroup', (data) => {
            ...
            if (this.tag) {
                // Determine if the received is from the current group chat, not just mark the red dot, but draw the chat content
                ...
                // Currently on the personal chat page, the group chat prompts new messages, and the corresponding group chat in the group chat list prompts new messages
